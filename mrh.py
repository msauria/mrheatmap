...
class MrhSquareDataProvider( base.LimitedOffsetDataProvider ):
    ...
    def __init__( self, source, chrom1='', start1=0, stop1=0, chrom2='', start2=0, stop2=0, minresolution=0,
                  maxresolution=0, header=False, chromosomes=False, **kwargs ):
        """
        ...
        self.start1 = max( 0, int( round( float( start1 ) ) ) )
        self.start2 = max( 0, int( round( float( start2 ) ) ) )
        self.stop1 = int( round( float( stop1 ) ) )
        self.stop2 = int( round( float( stop2 ) ) )
        self.minres = int( round( float( minresolution ) ) )
        self.maxres = int( round( float( maxresolution ) ) )
        if self.minres != self.maxres:
            self.levels = {}
            i = self.minres
            j = 0
            while i >= self.maxres:
    ...
    def paint_trans_canvas( self, infile ):
        ...
        for i in range( start_pos1, end_pos1 ):
            ...
            infile.seek( self.header[ 'offset' ] + ( self.header[ 'data_bins' ] + self.header[ 'index_bins' ] ) * 4 +
                         pos_index * 2 )
            shapes = struct.unpack( 'h' * ( end_pos2 - start_pos2 ), infile.read( ( end_pos2 - start_pos2 ) * 2 ) )
    ...
    def interpolate_square( self, square ):
        ...
        xspan = min( square[0] + square[2] - x1, self.window1 )
        y1 = max( square[1], self.start2 )
        yspan = min( square[1] + square[2] - y1, self.window2 )
